#!/usr/bin/env python3
"""
LLM-based entity resolution using Claude — with surname-prefix blocking.

This is a variant of llm_er.py. The only behavioural difference is how records
are packed into batches:

  * Records are first grouped by data source (no cross-source batches).
  * Within each source, records are grouped by a "blocking key": the first N
    characters of the normalized last name (default N=4, configurable via
    --block-prefix).
  * Each block is kept intact in a single batch when it fits in the token
    budget. Multiple small blocks from the same source can share a batch.
  * Blocks whose total size exceeds the token budget are split across batches.

Rationale: in the original llm_er.py, records were interleaved across batches
to mix data sources. That structurally guaranteed that within-source duplicates
("ANDREW COHEN" / "ANDREW COHEN" in EQUIFAX) often landed in different batches
and were invisible to each other. Surname-prefix blocking puts likely-matching
records into the same batch, which is how production ER systems work.

Trade-offs vs llm_er.py:
  * Cross-source matches are NOT attempted. The original was already finding
    almost no cross-source matches; this script makes that explicit.
  * For very common surname prefixes (e.g. SMIT, JOHN), a single block may be
    larger than the token budget and will be split — those splits can still
    miss matches inside the oversized block.

Usage:
    python llm_er_blocked.py data/equifax-small.csv data/npi-small.csv
    python llm_er_blocked.py data/equifax-lg.csv data/npi-lg.csv \\
        --token-budget 300000 --block-prefix 4
"""
...
# Default number of leading-name characters used for blocking
DEFAULT_BLOCK_PREFIX = 4
...
def extract_last_name(rec):
    """Pull a last-name-like string for blocking. Returns uppercase A-Z only."""
    # Top-level last-name fields (covers CSV-flattened records)
    for k in ("PRIMARY_NAME_LAST", "NAME_LAST"):
        v = rec.get(k)
        if v:
            return _NON_ALPHA.sub("", str(v).upper())

    # Nested FEATURES list (covers JSONL records)
    ...
    # Free-form name fallbacks: take the last whitespace token
    for k in ("NAME_FULL", "NAME_ORG"):
        v = rec.get(k)
        if v:
            tokens = str(v).split()
            if tokens:
                return _NON_ALPHA.sub("", tokens[-1].upper())

    return ""


def block_key(rec, prefix_len):
    """Cheap blocking key: first prefix_len chars of normalized last name.

    Records with no extractable name go to a shared "_NONAME_" block so they
    are grouped together rather than scattered.
    """
    last = extract_last_name(rec)
    if not last:
        return "_NONAME_"
    # Pad short surnames with '_' so e.g. "WU" and "WUSE" don't collide
    return last[:prefix_len].ljust(prefix_len, "_")
...
def build_batches(all_records, token_budget, block_prefix_len=DEFAULT_BLOCK_PREFIX):
    """
    Group records by data source and surname-prefix block, then pack into batches.

    Rules:
      * Each batch contains records from a single data source (no cross-source).
      * A "block" = all records from one source sharing the first N chars of
        their normalized last name. Blocks are kept intact within one batch
        when they fit in the budget.
      * Multiple blocks from the same source can share a batch as long as the
        combined size fits.
      * If a single block exceeds the budget it is split across batches.
    """
    # Group by (data_source, block_key)
    blocks = defaultdict(list)
    for rec in all_records:
        ds = rec.get("DATA_SOURCE", "UNKNOWN")
        key = block_key(rec, block_prefix_len)
        blocks[(ds, key)].append(rec)

    system_tokens = estimate_tokens(SYSTEM_PROMPT)
    boilerplate_tokens = 300
    overhead = system_tokens + boilerplate_tokens

    def csv_row_tokens(rec):
        return estimate_tokens(",".join(str(v) for v in rec.values()))

    # Pre-compute per-record sizes once
    rec_sizes = {id(r): csv_row_tokens(r) for r in all_records}

    def block_size(recs):
        return sum(rec_sizes[id(r)] for r in recs)

    # Group blocks by data source, sorted by size descending for tighter packing
    by_source = defaultdict(list)
    for (ds, key), recs in blocks.items():
        by_source[ds].append((key, recs, block_size(recs)))
    for ds in by_source:
        by_source[ds].sort(key=lambda x: -x[2])

    batches = []
    split_blocks = 0  # count of blocks that had to be split

    for ds in sorted(by_source.keys()):
        current_batch = []
        current_tokens = overhead

        for key, recs, total_tok in by_source[ds]:
            # Oversized block — must be split across multiple batches
            if total_tok + overhead > token_budget:
                split_blocks += 1
                if current_batch:
                ...
                for rec in recs:
                    sz = rec_sizes[id(rec)]
                    if tmp and (tmp_tok + sz > token_budget):
                        batches.append(tmp)
                        tmp = [rec]
                        tmp_tok = overhead + sz
                    else:
            ...
            # Block fits as a unit — try to add to current batch
            if current_batch and (current_tokens + total_tok > token_budget):
                batches.append(current_batch)
                current_batch = list(recs)
                current_tokens = overhead + total_tok
            else:
    ...
    # Print a quick summary of the blocking
    block_sizes_list = [len(recs) for recs in blocks.values()]
    print(
        f"  Blocking: {len(blocks)} blocks (prefix={block_prefix_len}). "
        f"Largest block: {max(block_sizes_list) if block_sizes_list else 0} records. "
        f"Split blocks: {split_blocks}"
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="LLM-based entity resolution using Claude with surname-prefix blocking."
    )
    ...
    parser.add_argument(
        ...
        "-o",
        default=None,
        help="Output file path (default: llm_er_output_blocked.jsonl in input dir)",
    )
    parser.add_argument(
        "--max-tokens",
        type=int,
    ...
    parser.add_argument(
        ...
        help=f"Number of leading last-name characters to use as the blocking key "
             f"(default: {DEFAULT_BLOCK_PREFIX}).",
    )
    ...
    # Build batches (single-source, surname-blocked)
    batches = build_batches(all_records, args.token_budget, args.block_prefix)
    ...
    if num_batches == 1:
        ...
        print(f"\nSplit into {num_batches} batches "
              ...
              f"block prefix: {args.block_prefix})")
    ...
    output_path = args.output or os.path.join(
        os.path.dirname(args.file1) or ".", "llm_er_output_blocked.jsonl"
    )
